...
update_dashbase_valuefile() {
  ...
  if [ "$SYSTEM_LOG" == "true" ]; then
    ...
    if [ "$VNUM" -ge 2 ]; then
       kubectl exec -it admindash-0 dashbase -- sed -i '/V1_tables/ r /data/dashbase_system_log_table_v2_poc.yaml' /data/dashbase-values.yaml
    else
       kubectl exec -it admindash-0 dashbase -- sed -i '/Dashbase_Logs/ r /data/dashbase_system_log_table_v1_poc.yaml' /data/dashbase-values.yaml
    fi
  fi
