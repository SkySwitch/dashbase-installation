...
download_dashbase() {
  ...
  if [ "$BASIC_AUTH" == true ]; then
    ...
    kubectl exec -it admindash-0 -n dashbase -- bash -c "wget -O /data/dashbase-values.yaml https://github.com/dashbase/dashbase-installation/raw/master/deployment-tools/dashbase-admin/dashbase_setup_tarball/tinysetup/dashbase-values-basicauth.yaml"
  else
